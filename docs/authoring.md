...
plugins/<domain>/skills/<name>/
  ...
  evals/rubric.json         # trigger cases + acceptance expectations (required)
  evals/result.json         # the recorded measurement (written by the eval, not authored)
...
- Examples are **fictional and generic** — never paste a real work identifier into a skill's examples
  (domain skills that describe a specific system are the exception).
...
## evals/rubric.json (what the skill is measured against)

One object, two halves. `trigger` is a list of natural-language queries, each labeled
whether the skill's `description` should fire — cover both sides, real triggers and
near-miss non-triggers. `acceptance` is the list of expectations the RESULT must satisfy
(or `{"not-applicable": "<reason>"}` for a skill that produces nothing to check):
...
```json
{
  ...
  "acceptance": [
    "an expectation the skill's output must meet",
    "another checkable expectation"
  ]
}
```
...
```bash
python3 scripts/validate_eval.py plugins/<domain>/skills/<name>/evals/rubric.json
```
...
A plugin's hooks live in `plugins/<domain>/hooks/` with a `hooks.json` that names the event
(`SessionStart`, `UserPromptSubmit`, `PreToolUse`, `PostToolUse`) and the command to run.

**Write hooks shell-first.** `UserPromptSubmit`, `PreToolUse` and `PostToolUse` fire on the hot path
— every message or every edit — and block synchronously, so interpreter startup is paid per fire.
Pick the language in this order, stopping at the first that fits:

```text
bash → sh → node → python (last resort)
```

- **bash / sh** for simple path/string logic (read one `file_path`, match a prefix or suffix, print
  a note). Fastest, no build step.
- **node** only for the two cases bash cannot do cleanly: unicode codepoint classification (macOS
  `grep` lacks `\p{Script}`), and real JSON-in/JSON-out plus git/subprocess logic (where bash would
  need `jq`). node is also the choice if a hook must run on native Windows, where bash is absent.
- **python** only if nothing above fits — slowest to start, not guaranteed on Windows.

When rewriting a hook's language: parity-test the replacement against the original (same output, same
exit code across a scenario matrix), then delete the old script and update `hooks.json` in the **same**
change — never leave a dangling reference. Full rationale and the measured numbers: `docs/adr/0002`.

## Before you commit

- Every touched skill has an up-to-date `metadata.yaml` and a valid `rubric.json`.
- `python3 scripts/gen_catalog.py` runs clean.
- Update `README.md`'s Skills section by hand if you added or renamed a skill.
- `cerberus:leak-check` (real-identifier scrub) and `cerberus:security-scan` (eight-point consumer-safety check) run automatically on skill edits — clear any flag either raises
  before shipping.
