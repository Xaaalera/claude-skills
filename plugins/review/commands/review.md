...
7. On OVERALL PASS: writing and committing the attestation is the IMMEDIATE next action — do it FIRST, before ANY other edit. **Never touch code between a green verdict and the committed attestation.** Any edit — even fixing a reported finding — changes the diff hash and VOIDS the review, forcing a re-run; address findings only AFTER the attestation is committed, as a separate change that gets its own `/review`. ALWAYS write and commit the attestation — automatically, without asking. This is not optional: the committed `.review/attestations/<diffHash>.json` is the branch anchor that lets the NEXT `/review` short-circuit (step 0) instead of re-running the whole cycle. Build the per-agent JSON `{ "<agent>": {"score":N,"verdict":"PASS"}, ... }` (INCLUDING the carried-forward verdicts from step 0) and write it:
