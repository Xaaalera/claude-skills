---
description: House pattern for surfacing Apex errors from a Salesforce package with a uniform, code-based error contract. Use when adding or handling any Apex error path — throwing from a controller/REST resource/data-sync layer, or when a raw platform error (e.g. "Variable does not exist: tmpVar1") leaks to a client. Establishes a code registry + envelope and keeps it updated.
---
...
3. **Normalize at the transport entry points** (data-sync layer + REST resources), not per-query — one
...
Reference implementation: <sf-repo> `ApiErrors.cls` + `docs/ERROR_CODES.md`.
