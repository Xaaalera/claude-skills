...
`evals/result.json`). Refresh a measurement by running the optimizer from this repo,
pointing it at the skill's path:
`python3.14 scripts/optimize_description.py --skill-path <dir> --apply`, which writes
`evals/result.json`. (`scripts/` holds the eval runner + validator + scorer.)
