---
description: Prefer the salesforce-dx MCP tools over raw `sf` CLI for ANY interaction with a Salesforce org — SOQL/Tooling queries, running Apex tests, deploying or retrieving metadata. Use whenever you query the org, run tests, or push/pull metadata.
---
...
Activate before ANY action that touches a Salesforce org, in either repo
(`AccountingCloud` or `Accounting-Seed-UI`):
...
- Deploying metadata to the org / retrieving metadata from the org
- Resolving which org/username to use, or listing orgs
- Resuming a long-running deploy/test/scratch-org job

This OVERRIDES the `sf` CLI examples in `CLAUDE.md` (`sf data query`,
`sf apex run test`, `sf project retrieve start`). Reach for the MCP first; fall
back to `sf` via Bash only when an MCP tool genuinely can't do the job (e.g.
`sf org login`, scratch-org creation, an interactive flag).
...
- **No permission prompts** for raw shell, and consistent behavior across sessions.
...
call in a session, fetch their schemas:
...
Only load the ones you need. After they appear in a `<functions>` block they are
...
org/config you want, e.g. `/Users/roman/work/AccountingCloud`. Keep using the
same directory across calls unless the task moves to another repo.
...
an alias. In this environment the default target org is **`acountingSeed`**
(note the spelling — one `c`), and the AcctSeed namespace requires
`/services/apexrest/AcctSeed/...` for Apex REST.

## Gotcha #4 — Tooling API for metadata queries

For `ApexClass`, `CustomMetadata` / `*__mdt` (e.g. `UI_Default_KPI_Card__mdt`),
`ApexTestResult`, and similar, set `useToolingApi: true` on `run_soql_query`.
...
| Tool | Use for |
|---|---|
| `run_apex_test` | Run Apex tests. `testLevel: "RunSpecifiedTests"` + `classNames` (and/or `methodNames`) for our classes; `RunLocalTests` to skip managed pkgs; `RunAllTestsInOrg` for everything. Add `codeCoverage: true` for coverage, `verbose: true` to see passing/failure detail, `async: true` to get a `testRunId` and read results later. |
...
| `run_agent_test` | Agentforce tests only — not used for Apex. |

---

## Typical Apex-test loop (FS-14702 and similar)
